...
Subscribes to three Vectornav IMU topics and displays variance ellipsoids.
...
Expects three topics publishing Vectornav.msg:
    /imu1/vectornav
    /imu2/vectornav
    /imu3/vectornav

Custom message format (Vectornav.msg):
...
# Import custom message from vn_driver package
try:
    from vn_driver.msg import Vectornav
    print("[INFO] Successfully imported Vectornav from vn_driver.msg")
except ImportError as e:
    print(f"[ERROR] Failed to import Vectornav from vn_driver.msg: {e}")
    print("Make sure you have sourced your workspace: source ~/your_ws/install/setup.bash")
    Vectornav = None
...
class IMURos2Subscriber(Node):
    """ROS2 node that subscribes to three Vectornav IMU topics."""
    ...
    def __init__(self, data_callback, topic_names: List[str]):
        ...
        # Try to match common QoS profiles - using RELIABLE which is more common for default publishers
        # If your publisher uses BEST_EFFORT, change this to BEST_EFFORT
        qos_profile = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,  # Try RELIABLE first (default for most publishers)
        ...
        for topic_name, topic_types in topic_list:
            if 'vectornav' in topic_name.lower() or 'imu' in topic_name.lower():
    ...
    def imu_callback(self, msg: Vectornav, sensor_id: int):
        """Callback for IMU messages."""
        try:
            # Extract timestamp from header
            timestamp = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9
            
            # Create IMU sample from message
            sample = IMUSample(
                timestamp=timestamp,
...
class IMUComparatorGUI:
    ...
    def __init__(self, root):
        ...
        # Parameters
        self.update_interval = 100  # ms (10Hz update rate)
    ...
    def start_monitoring(self):
        ...
        if Vectornav is None:
            messagebox.showerror("Error", 
                "Vectornav message type not found.\n"
                "Please ensure your custom message package is built and sourced.\n\n"
                "Example:\n"
                "  colcon build --packages-select <your_msg_package>\n"
...
def main():
    """Main function."""
    # Check for Vectornav message type
    if Vectornav is None:
        print("\n" + "="*60)
        print("WARNING: Vectornav message type not available!")
        print("="*60)
        print("\nTo use this script, ensure your custom message package")
        print("is built and sourced. The script expects a message with:")
        print("  - std_msgs/Header header")
        print("  - geometry_msgs/Vector3 linear_acceleration")
        print("  - geometry_msgs/Vector3 angular_velocity")
        print("  - geometry_msgs/Vector3 magnetic_field")
        print("\nExample setup:")
        print("  1. Build your message package:")
        print("     colcon build --packages-select <your_msg_package>")
        print("  2. Source the workspace:")
        print("     source install/setup.bash")
        print("  3. Run this script")
        print("\nContinuing with GUI (will show error on start)...")
        print("="*60 + "\n")
